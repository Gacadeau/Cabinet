...
export async function POST(req) {
  try {
    ...
    // Mettez à jour le dossier existant en définissant `penalite` à 1
    await executeQuery('UPDATE dossiers SET penalite = ? WHERE id = ?', [1, id_dossier]);
    ...
    await executeQuery(
      'INSERT INTO penalite (dossier, penalite, comment) VALUES (?, ?, ?)',
