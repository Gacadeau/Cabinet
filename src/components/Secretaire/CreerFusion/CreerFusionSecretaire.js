...
const CreerFusionSecretaire = ({closeModal,item}) => {
    ...
    const [formFusionData, setFormFusionData] = useState({
      ...
      const handleFusionFormSubmit = async (e) => {
        ...
        try {
          const response = await fetch('/api/dossiers/fusion', {
            method: 'POST',
            headers: {
          ...
          alert('Fusion créée avec succès!');
          closeModal();
        } catch (error) {
          console.error('Error:', error);
          alert('Une erreur est survenue lors de la création de la Fusion.');
        } finally {
  ...
  return (
        <div className='mx-auto w-[100%] h-[100%] sm:h-[95%] md:h-[68%] lg:h-[100%] flex lg:items-center bg-slate-700 bg-opacity-25 justify-center'>
            <form onSubmit={handleFusionFormSubmit} className='w-[95%] md:w-[50%] lg:w-[30%] md:h-[97%] lg:h-[97%] overflow-y-scroll mt-2 md:mt-2 mb-2 md:mb-0 relative flex flex-col space-y-4 bg-white rounded-md rounded-tr-2xl'>
            ...
            <span className='flex justify-center text-lg font-bold md:text-2xl lg:text-3xl md:font-bold text-slate-700 mt-5'>Creez une fusion</span>
...
export default CreerFusionSecretaire
