...
const CreerPenaliteSecretaire = ({ closeModal, item }) => {
  ...
  const handlePenaliteFormSubmit = async (e) => {
    ...
    try {
      const resTribunal = await fetch('/api/tribunal', {
        ...
        body: JSON.stringify({ nom: formPenaliteData.tribunal }),
      });
      const tribunal = await resTribunal.json();
      const tribunalId = tribunal.id;

      const response = await fetch('/api/dossiers/penalite', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          demandeur: formPenaliteData.demandeur,
          defendeur: formPenaliteData.defendeur,
      ...
      alert('Pénalité créée avec succès!');
      closeModal();
    } catch (error) {
      console.error('Error:', error);
      alert('Une erreur est survenue lors de la création de la pénalité.');
    } finally {
        ...
        <div className='w-[78%] flex flex-col space-y-3 mx-auto'>
          ...
          <div className='flex flex-col space-y-2 w-full'>
            ...
            <input
              type='text'
              name='demandeur'
              value={formPenaliteData.demandeur}
              onChange={handlePenaliteFormChange}
          ...
          <div className='flex flex-col space-y-2 w-full'>
            ...
            <input
              type='text'
              name='defendeur'
              value={formPenaliteData.defendeur}
              onChange={handlePenaliteFormChange}
              className='border border-slate-700 text-slate-700 text-lg rounded-lg block w-full p-1.5 md:p-2.5 dark:placeholder-slate-700 focus:outline-none'
              required
            />
          ...
          <div className='flex flex-col space-y-2 w-full'>
            ...
            <input
              type='date'
              name='date_audience'
              value={formPenaliteData.date_audience}
              onChange={handlePenaliteFormChange}
              className='border border-slate-700 text-slate-700 text-lg rounded-lg block w-full p-1.5 md:p-2.5 dark:placeholder-slate-700 focus:outline-none'
              required
            />
...
export default CreerPenaliteSecretaire;
